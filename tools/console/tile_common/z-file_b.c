 ...
 * Modified 2011 by Brett Reid to use FILE pointers directly, for tool
 *  programs outside of the game, this file should not be mixed with
 *  or replace a file in the game source.
 *
 ...
 */
//#define ZNEW(x) (x*)malloc(sizeof(x));
 ...
 */
FILE *file_open(const char *fname, file_mode mode, file_type ftype)
{
  ...
  FILE *fh;
	...
	switch (mode)
	{
		case MODE_WRITE:  fh = fopen(buf, "wb"); break;
		case MODE_READ:   fh = fopen(buf, "rb"); break;
		case MODE_APPEND: fh = fopen(buf, "a+"); break;
		default:          fh = fopen(buf, "__");
	}
 ...
 */
bool file_seek(FILE *fh, u32b pos)
{
 ...
 */
bool file_readc(FILE *fh, byte *b)
{
	int i = fgetc(fh);
 ...
 */
bool file_writec(FILE *fh, byte b)
{
	return file_write(fh, (const char *)&b, 1);
}

/*
 * Read 'n' bytes from file 'f' into array 'buf'.
 */
int file_read(FILE *fh, char *buf, size_t n)
{
	size_t read = fread(buf, 1, n, fh);
 ...
 */
bool file_write(FILE *fh, const char *buf, size_t n)
{
	return fwrite(buf, 1, n, fh) == n;
}
...
bool file_getl(FILE *fh, char *buf, size_t len)
{
	bool seen_cr = FALSE;
	byte b;
	size_t i = 0;
 ...
 * line ending.
 */
bool file_put(FILE *fh, const char *buf)
{
	return file_write(fh, buf, strlen(buf));
}
 ...
 */
bool file_putf(FILE *fh, const char *fmt, ...)
{
	va_list vp;
	bool status;
 ...
 */
bool file_vputf(FILE *fh, const char *fmt, va_list vp)
{
	char buf[1024];
	...
	return file_put(fh, buf);
}

/*
 * Format and translate a string, then print it out to file.
 */
bool x_file_putf(FILE *fh, int encoding, const char *fmt, ...)
{
	va_list vp;
